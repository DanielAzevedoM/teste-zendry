...
<template>
  <div>
    <div class="d-flex flex-column ga-6 mb-6">
      <div v-if="pixTransaction" class="text-center">
        <v-sheet border rounded class="pa-4">
          ...
          <p class="text-body-2 mb-3">Aponte a câmera do seu celular para o QR Code abaixo ou utilize o código "copia e
            cola".</p>

          <v-img v-if="pixTransaction.serverData.qrCodeImage" :src="pixTransaction.serverData.qrCodeImage" width="220"
            height="220" class="mx-auto my-2 elevation-2 rounded" />

          <CountdownTimer v-if="status === 'processing' && pixTimerMinutes > 0" :minutes="pixTimerMinutes"
            countdown-text="Este código expira em:" class="my-3" />

          <v-text-field readonly :model-value="pixTransaction.serverData.qrCode" label="PIX Copia e Cola"
            append-inner-icon="mdi-content-copy" @click:append-inner="copyPixCode" variant="outlined" class="mt-4" />
        </v-sheet>
      </div>
      ...
      <div v-if="boletoTransaction" class="text-center">
        <v-sheet border rounded class="pa-4">
          ...
          <v-text-field readonly :model-value="boletoTransaction.serverData.linhaDigitavel" label="Linha Digitável"
            append-inner-icon="mdi-content-copy" @click:append-inner="copyBoletoCode" variant="outlined" class="mt-4" />

          <VBtn :href="boletoTransaction.serverData.boletoUrl" target="_blank" color="success" variant="elevated"
            size="large" block class="mt-3">
    ...
    <v-alert :type="status === 'approved' ? 'success' : 'info'" variant="tonal" border="start" class="mb-4">
      <template #default>
