...
<template>
  <VForm ref="formRef" v-model="isFormValid" validate-on="input" class="v-form-style mt-5">
    ...
    <v-row>
      ...
      <v-col>
        <VTextField v-model="formData.city" label="Cidade" variant="outlined"
    ...
    <VTextField v-model="formData.street" label="Rua" variant="outlined" :rules="[rules.required('Rua')]" />
    ...
    <v-row>
      ...
      <v-col>
        <VSelect v-model="formData.state" label="Estado" :items="stateOptions" item-title="name"
          item-value="code" :disabled="!formData.country || stateOptions.length === 0" variant="outlined"
          density="comfortable" :rules="formData.country ? [rules.required('Estado')] : []" />
      </v-col>
    </v-row>
  </VForm>
</template>
